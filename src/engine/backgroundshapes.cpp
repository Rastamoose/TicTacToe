...
BackgroundShapes::BackgroundShapes(const Point& screenSize, int pquantity, float prowDelay, float pspeed, const Colour& pbgColour, const Colour& pcolour1, const Colour& pcolour2, int pwidth, int pthickness):
    quantity(pquantity), rowDelay(prowDelay), speed(pspeed), colours(pbgColour, pcolour1, pcolour2), width(pwidth),
    thickness(pthickness), time(0.0), randGenTime(randint(0,prowDelay)), shapeGapW(0.5){
    initGrid(screenSize);
}
...
std::unique_ptr<Shape> BackgroundShapes::initRandShape(Point pos, int shapeSpeed, std::pair<int, int> columnPos){
    ...
    switch(shapeType){
        case CROSS:            
            shape = std::make_unique<Cross>(Cross(width, pos, shapeSpeed, colours.shape1, thickness, columnPos));            
            break;

        case CIRCLE:
            shape = std::make_unique<Circle>(Circle(width, pos, shapeSpeed, colours.shape2, thickness, columnPos));
            break;
...
void BackgroundShapes::genShapes(const Point& size, int yPos){
    ...
    for(int i=0; i<quantity; i++){
        /* make sure shape doesnt collide with any other on its way down 
         * by organising shapes in their columns (if there is no space for 
         * shape in column then shape will not be generated) */
    ...
    for(auto &row:shapes){
        ...
        for(std::deque<std::deque<std::unique_ptr<Shape> > >::size_type i=0; i<row.size(); i++){
            ...
            if (shape->pos.y > display->getSize().y){                        // check to see if shape has gone off screen
                ...
                display->setDrawColour(colours.bg);                                         // set bg colour
...
void Cross::update(DisplayManager* display){
    // draw diagonals across rect
    display->drawThickLine(Point(pos.x,pos.y),Point(pos.x+width,pos.y+width), thickness, colour);
    display->drawThickLine(Point(pos.x,pos.y+width),Point(pos.x+width,pos.y), thickness, colour);
}
